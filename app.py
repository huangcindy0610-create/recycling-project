...
def get_local_chart_data():
    ...
    monday = (datetime.now() - timedelta(days=datetime.now().weekday())).replace(hour=0, minute=0, second=0, microsecond=0)
    db = get_db()
    # 按照時間順序排列，讓顯示更直觀
    ...
    for r in rows:
        try:
            ...
            if start_dt >= monday:
                diff = (datetime.strptime(r['endtime'], '%Y-%m-%d %H:%M:%S') - start_dt).total_seconds()
...
def home():
    ...
    lvl = get_level(xp)
    title = get_current_character(lvl) # 🌱 稱號獲取
    chart, sessions = get_local_chart_data()
...
def submit_answer():
    ...
    return jsonify({'correct': ans == correct, 'gained_xp': gained, 'new_level': get_level(new_xp), 'explanation': session.get('explanation'), 'correct_answer': correct})
