...
def home():
    ...
    return render_template('demo_baby_v4.html', username=u, xp=xp, level=lvl, title=title, chart_data=chart_data, sessions_data=sessions_data)
...
def register():
    ...
    if p != cp: return render_template('login.html', error='兩次密碼輸入不一致')
